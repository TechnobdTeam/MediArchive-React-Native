...
export default class EditReportScreen extends Component {
  ...
  constructor(props){
    ...
    image_name: '',
    name: '',
    doctor_name: this.props.prescribe_by,
  ...
  componentDidMount() {
    ...
    console.log(this.props.report_photo, this.props.image_list, )
    ...
    AsyncStorage.getItem(AppConstant.jwt_token, (error, values) => {
      ...
      if (this.state.day === '' || this.state.day === 'Day'){
        this.getApiResponse('getReportInfo', '');
      }
  ...
  getYears() {
      ...
      this.timeoutHandle = setTimeout(() => {
        ...
        ImagePicker.launchImageLibrary(options, (response) => {
          ...
          if (response.didCancel) {
            ...
            customImageArray.push(image_custom)

            this.setState({
              image_list: this.state.image_list.length === 0 ? imageArray : [...this.state.image_list, ...imageArray],
              report_photo: this.state.report_photo.length === 0 ? customImageArray : [...this.state.report_photo, ...customImageArray],
            })

            console.log(' image_list: ' + this.state.image_list.length, ' , ', this.state.report_photo.length)
          }
    ...
    requestImage = () => {
      ...
      console.log("image_uri: " + this.state.action_type);
      ...
      if (this.state.action_type==='edit'){
        formData.append('report_id', this.props.report_id);
      }else{
        formData.append('patient_id', this.state.patient_id);
      }

      console.log('---------1 device_uuid: ' + device_uuid 
      + " Name: " + this.state.report_type 
      + ' dob_obj : ' + dob_obj
      +' URL: ' + URL,
      ' this.props.report_id:' + this.props.report_id,
      " this.state.action_type: " + this.state.action_type)
  ...
  console.log(" ######## type:" + api_type, search);
  ...
  if(api_type==='doctor_list'){
  ...
  } else if (api_type === 'getReportInfo') {
    URL = AppConstant.BASE_URL + "prescription/getReportInfo";
    ...
    console.log(api_type ," file_id:" + this.state.file_id);
  }
      ...
      return fetch(URL, {
        ...
        .then((responseJson) => {
          ...
          console.log(responseJson);
            ...
            if (api_type === 'getReportInfo') {
            if (responseJson.response.type === "success") {

                if (responseJson.response.data.prescribe_info.date != '') {
                  const str = responseJson.response.data.prescribe_info.date;
                  const res = str.split(' ');
                  var monthId = this.getMonthId(res[1])
                ...
                this.setState({
                  isLoading: false,
                  report_type: responseJson.response.data.prescribe_info.type,
                  description: responseJson.response.data.description,
                  doctor_name: responseJson.response.data.prescribe_info.prescribe_by,
...
renderSeparator = () => {
  ...
  getIndex(id) {
    return this.state.image_list.findIndex(obj === id);
  }
    ...
    console.log(' before ', this.state.image_list.length)
      ...
      if (!item.photo.includes('https')) {
        if (this.state.image_list.length > 0) {
          ...
          var index=0;
          for (let i = 0; i < this.state.image_list.length; i++) {
          if (item.photo === this.state.image_list[i].image_uri) {
            console.log(' !!!!!!!match!!!!! ', i, ' : ', item)
            index = i;
            break;
            }
          }
          
          image_item = this.state.image_list[index]

          this.setState({
            image_list: this.removeFromArray(this.state.image_list, image_item),
            report_photo: this.removeFromArray(this.state.report_photo, item)
          })
        }
      } else {
        this.setState({
        ...
        this.timeoutHandle = setTimeout(() => {
          console.log(' After file_id: ', this.state.file_id)
          this.getApiResponse('deletePhoto', '')
        }, 200);
  ...
  fileScanning() {
    ...
    AppConstant.r_report_photo = this.state.report_photo
    AppConstant.r_report_id = this.state.report_id
  ...
  render(){
    const {width, height} = Dimensions.get('window');
      ...
      <Right style={{flex:1}}>
        ...
        {/* <Button Button onPress = {() => { this.checkAllValues()}} transparent >
